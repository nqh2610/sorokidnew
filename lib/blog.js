...
/**
 * 📅 Check if a post should be publicly visible
 * - Must have status = 'published'
 * - Must have publishedAt date <= current date
...
function isPostPubliclyVisible(post) {
  ...
  // If no publishedAt, it's visible (backward compatible)
  if (!post.publishedAt) {
    return true;
  }
  
  // Check if publishedAt is in the past or today
  const publishDate = new Date(post.publishedAt);
  const now = new Date();
  
  // Set both to start of day for comparison (ignore time)
    ...
    if (!fs.existsSync(filePath)) {
      ...
      if (locale === 'vi') {
        ...
        if (fs.existsSync(fallbackPath)) {
          ...
          // Check scheduled publishing (status + date)
          if (!isPostPubliclyVisible(post)) return null;
          if (!includeContent) {
...
/**
 * Lấy tất cả bài viết (chỉ published và đã đến ngày publish)
