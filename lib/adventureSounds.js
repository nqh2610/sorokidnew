...
export const ADVENTURE_SOUNDS = {
  ...
  // 🗺️ Mở map / vào map - Mysterious reveal
  mapOpen: (ctx, time) => {
    const notes = [392, 494, 587]; // G4, B4, D5 - Mystical chord
    notes.forEach((freq, i) => {
      ...
      gain.gain.setValueAtTime(0.12, time + i * 0.05);
  ...
  stageSelect: (ctx, time) => {
    ...
    osc.frequency.exponentialRampToValueAtTime(1100, time + 0.06);
  ...
  // 🔓 Mở khóa stage mới - Unlock chime
  ...
  correct: (ctx, time) => {
    ...
    gain.gain.setValueAtTime(0.25, time + 0.16);
    gain.gain.exponentialRampToValueAtTime(0.01, time + 0.28);
    osc.start(time);
    osc.stop(time + 0.28);
  },

  // ❌ Trả lời sai - Gentle low (không harsh)
  wrong: (ctx, time) => {
    const osc = ctx.createOscillator();
    const gain = ctx.createGain();
    osc.connect(gain);
    ...
    osc.frequency.exponentialRampToValueAtTime(165, time + 0.12);
  ...
  combo: (ctx, time) => {
    [0, 0.04, 0.08].forEach((delay, i) => {
      ...
      gain.connect(ctx.destination);
      osc.type = 'sine';
      osc.frequency.setValueAtTime(1000 + i * 200, time + delay);
      gain.gain.setValueAtTime(0.1, time + delay);
      gain.gain.exponentialRampToValueAtTime(0.01, time + delay + 0.06);
  ...
  // 🎯 Hoàn thành stage - Stage clear fanfare
  stageComplete: (ctx, time) => {
    ...
    melody.forEach((freq, i) => {
      ...
      gain.gain.setValueAtTime(0.18, time + i * 0.1);
  ...
  // 👹 Đánh bại Boss - Epic victory
  bossDefeat: (ctx, time) => {
    // Power chord + victory fanfare
    const chords = [
      { notes: [261, 329, 392], delay: 0, duration: 0.3 },      // C major
      { notes: [349, 440, 523], delay: 0.35, duration: 0.3 },   // F major
      { notes: [392, 494, 587, 784], delay: 0.7, duration: 0.5 } // G major + high
    ];
    ...
    chords.forEach(({ notes, delay, duration }) => {
      notes.forEach(freq => {
        ...
        gain.gain.exponentialRampToValueAtTime(0.01, time + delay + duration);
        osc.start(time + delay);
