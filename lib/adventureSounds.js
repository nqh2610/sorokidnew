...
export const ADVENTURE_SOUNDS = {
  ...
  // 🌟 Trả lời đúng NHANH - "Tuyệt vời!" sparkle fanfare
  correctFast: (ctx, time) => {
    // Sparkle arpeggio đi lên - nghe như "Amazing!"
    ...
    setTimeout(() => {
      ...
      sparkle.type = 'sine';
      sparkle.frequency.value = 1319; // E6
  ...
  // ⭐ Trả lời đúng TỐT - "Giỏi lắm!" cheerful
  correctGood: (ctx, time) => {
    // 2-note happy chime
    ...
    notes.forEach((freq, i) => {
      ...
      const delay = i * 0.08;
      gain.gain.setValueAtTime(0, time + delay);
      gain.gain.linearRampToValueAtTime(0.1, time + delay + 0.02);
  ...
  // 👍 Trả lời đúng CHẬM - "Được rồi" gentle encouragement  
  correctSlow: (ctx, time) => {
    // Simple 2-note confirmation
    const osc = ctx.createOscillator();
    const gain = ctx.createGain();
    const filter = ctx.createBiquadFilter();
    ...
    osc.frequency.setValueAtTime(523, time + 0.1); // C5
    filter.type = 'lowpass';
    filter.frequency.value = 800;
  ...
  // 🎉 HOÀN THÀNH MÀN - Victory Fanfare (hấp dẫn, chúc mừng)
  levelComplete: (ctx, time) => {
    // Part 1: Triumphant intro (0-0.6s)
    const intro = [
      { freq: 392, delay: 0, dur: 0.15 },     // G4
      { freq: 494, delay: 0.12, dur: 0.15 },  // B4
      { freq: 587, delay: 0.24, dur: 0.15 },  // D5
      { freq: 784, delay: 0.36, dur: 0.25 },  // G5 (hold)
    ];
    ...
    intro.forEach(({ freq, delay, dur }) => {
      ...
      osc.frequency.value = freq;
      gain.gain.setValueAtTime(0, time + delay);
      gain.gain.linearRampToValueAtTime(0.15, time + delay + 0.02);
      gain.gain.setValueAtTime(0.12, time + delay + dur * 0.7);
      gain.gain.exponentialRampToValueAtTime(0.001, time + delay + dur);
      osc.start(time + delay);
      osc.stop(time + delay + dur);
    });
    
    // Part 2: Celebration chord (0.6s)
    setTimeout(() => {
      const chord = [523, 659, 784, 1047]; // C5-E5-G5-C6 (C major)
      chord.forEach((freq, i) => {
        ...
        gain.gain.linearRampToValueAtTime(0.1, t + i * 0.02 + 0.03);
    ...
    // Part 3: Final sparkle flourish (1.1s)
    setTimeout(() => {
      const sparkles = [1047, 1175, 1319, 1568]; // C6-D6-E6-G6
  ...
  // 🏅 HOÀN THÀNH XUẤT SẮC - Grand Victory (3 sao)
  levelCompletePerfect: (ctx, time) => {
    // Part 1: Heroic fanfare
    const fanfare = [
      { freq: 392, delay: 0, dur: 0.12 },
      { freq: 392, delay: 0.12, dur: 0.12 },
      { freq: 392, delay: 0.24, dur: 0.12 },
      { freq: 523, delay: 0.4, dur: 0.3 },  // C5 hold
    ];
    ...
    fanfare.forEach(({ freq, delay, dur }) => {
      ...
      gain.gain.setValueAtTime(0, time + delay);
      gain.gain.linearRampToValueAtTime(0.15, time + delay + 0.02);
      gain.gain.exponentialRampToValueAtTime(0.01, time + delay + dur);
      osc.start(time + delay);
      osc.stop(time + delay + dur);
    });
    
    // Part 2: Triumphant chord progression
    setTimeout(() => {
      // F major -> G major -> C major (classic victory cadence)
      const progressions = [
        { notes: [349, 440, 523], delay: 0 },      // F major
        { notes: [392, 494, 587], delay: 0.25 },   // G major
        { notes: [523, 659, 784, 1047], delay: 0.55 }, // C major (full)
      ];
      
      progressions.forEach(({ notes, delay }) => {
        setTimeout(() => {
          notes.forEach((freq, i) => {
            const osc = ctx.createOscillator();
            const gain = ctx.createGain();
            ...
            gain.gain.linearRampToValueAtTime(0.12, t + i * 0.015 + 0.02);
    ...
    // Part 3: Magic sparkles cascade
    setTimeout(() => {
      const cascade = [1047, 1175, 1319, 1397, 1568, 1760, 2093]; // C6 to C7
