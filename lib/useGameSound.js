...
export function useGameSound() {
  ...
  const playMusic = useCallback(async (theme = 'adventure') => {
    console.log('🎵 useGameSound.playMusic called:', theme, 'soundEnabled:', isSoundEnabled());
    
    if (!isSoundEnabled()) {
      console.log('🎵 Sound is disabled, skipping music');
      return;
    }
    if (musicPlaying && currentMusicTheme === theme) {
      console.log('🎵 Music already playing same theme');
      return;
    }
    ...
    try {
      ...
      if (!backgroundMusic) {
        console.log('🎵 Getting background music instance...');
        backgroundMusic = getBackgroundMusic();
      }
      
      // Use new background music system
      if (backgroundMusic) {
        console.log('🎵 Calling backgroundMusic.play...');
