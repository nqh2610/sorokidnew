 ...
 * Body: { slug: string, lang?: 'vi' | 'en', scheduledAt?: string }
 */
...
export async function POST(request) {
  try {
    ...
    const { slug, lang = 'vi', scheduledAt } = await request.json();
    ...
    // Nếu có scheduledAt thì set trạng thái scheduled
    if (scheduledAt) {
      post.status = 'scheduled';
      post.publishedAt = scheduledAt;
    } else {
      // Cập nhật status và publishedAt ngay lập tức
